...
func generateDummyData(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < count; i++ {
		item := map[string]*dynamodb.AttributeValue{
			...
			"ProductImage":       {SS: []*string{aws.String("image1.jpg"), aws.String("image2.jpg")}},
...
func deleteDummyData(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to scan Product: %s", err.Error()), http.StatusInternalServerError)
		return
	}
